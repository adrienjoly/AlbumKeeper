webAlbumDetector = new (function WebAlbumDetector(){
  ...
  var SOURCES = {
    "sc": new (function SoundCloudSetPageDetector(){
      this.findAlbumsFromNode = function(node){
        window = window || {};
        if (!window.location || window.location.href.indexOf("soundcloud.com") == -1)
          return [];
        var album = new WebAlbum();
        album.url = window.location.href;
        album.title = getNodeText(node.getElementsByClassName("soundTitle__title ")[0]).trim();
        album.artist = getNodeText(node.getElementsByClassName("soundTitle__username")[0]).trim();
        album.img = node.getElementsByClassName("image__full")[0].getAttribute("src");
        return [album];
      };
    })(),
  ...
  function WebAlbum(){
    ...
    this.tags = [];  // array of tags (extracted from desc)
  ...
  WebAlbum.prototype.consolidate = function(){
    ...
    this.tags = (this.desc || "").match(/\#([a-z][a-z0-9\-\_]+)/g).map(function(tag){
      return tag.substr(1); // remove the hash
    });
    return this;
  };
